--[[
    1NXITER UI LIBRARY v5.0 (FINAL)
    Estilo: Red Dark / Rounded / Big UI / Mobile Friendly
    Features: Abas, Toggles, Botões, Inputs, Sliders, Dropdowns, Notificações, Perfil.
]]
...
function Library:CriarJanela(NomeScript)
	...
	-- Limpeza de GUIs antigas para não duplicar
	...
	-- Tenta injetar no CoreGui (mais seguro), se falhar vai no PlayerGui
	...
	local SFix = Instance.new("Frame"); SFix.Size = UDim2.new(1,0,0,20); SFix.BackgroundTransparency=1; SFix.Parent=Sidebar
	...
	-- Lógica de Minimizar e Maximizar
	...
	function UI:CriarAba(Icone)
		...
		-- ==============================================================================
		-- 5. ELEMENTOS DA UI (COMPONENTES)
		-- ==============================================================================
		
		-- > TOGGLE (Checkbox Grande)
		function Tab:CriarToggle(Texto, Padrao, Callback)
			...
			local Box = Instance.new("TextButton"); Box.Size = UDim2.new(0, 26, 0, 26); Box.Position = UDim2.new(0, 2, 0.5, -13)
			Box.BackgroundColor3 = Theme.ItemBg; Box.Text = ""; Box.AutoButtonColor = false; Box.Parent = Container
			...
			Box.MouseButton1Click:Connect(function()
				Ativado = not Ativado
				Check.Visible = Ativado
				if Ativado then Stroke.Color = Theme.Accent else Stroke.Color = Theme.TextDim end
		...
		function Tab:CriarInput(Titulo, Padrao, Callback)
			...
			local BoxBg = Instance.new("Frame"); BoxBg.Size=UDim2.new(1,-10,0,32); BoxBg.Position=UDim2.new(0,0,0,22); BoxBg.BackgroundColor3=Theme.ItemBg; BoxBg.Parent=Container
			AddCorner(BoxBg, 8)
			
			local Box = Instance.new("TextBox"); Box.Text = Padrao; Box.Size=UDim2.new(1,-10,1,0); Box.Position=UDim2.new(0,10,0,0); Box.BackgroundTransparency=1
			Box.TextColor3=Theme.Text; Box.Font=Enum.Font.GothamBold; Box.TextXAlignment=Enum.TextXAlignment.Left; Box.Parent=BoxBg
		...
		function Tab:CriarDropdown(Titulo, Opcoes, Callback)
			...
			local MainBtn = Instance.new("TextButton"); MainBtn.Text = Opcoes[1] or "..."; MainBtn.Size=UDim2.new(1,-10,0,32); MainBtn.Position=UDim2.new(0,0,0,22); MainBtn.BackgroundColor3=Theme.ItemBg; MainBtn.TextColor3=Theme.Text; MainBtn.Font=Enum.Font.GothamBold; MainBtn.ZIndex = 6; MainBtn.Parent=DropFrame
			AddCorner(MainBtn, 6)
			
			local ListFrame = Instance.new("ScrollingFrame"); ListFrame.Size = UDim2.new(1, 0, 0, 0); ListFrame.Position = UDim2.new(0, 0, 1, 5); ListFrame.BackgroundColor3 = Theme.ItemBg; ListFrame.Visible = false; ListFrame.ZIndex = 10; ListFrame.Parent = MainBtn
			AddCorner(ListFrame, 6); AddStroke(ListFrame, Theme.Sidebar, 1)
			...
			MainBtn.MouseButton1Click:Connect(function()
				...
				if Aberto then
					...
					TweenService:Create(ListFrame, TweenInfo.new(0.2), {Size = UDim2.new(1, 0, 0, math.min(#Opcoes * 30, 120))}):Play()
				else
		...
		function Tab:CriarSlider(Titulo, Min, Max, Padrao, Callback)
			...
			local BarBg = Instance.new("TextButton"); BarBg.Text=""; BarBg.Size=UDim2.new(1,-10,0,6); BarBg.Position=UDim2.new(0,0,0,35); BarBg.BackgroundColor3=Theme.ItemBg; BarBg.AutoButtonColor=false; BarBg.Parent=Container
			AddCorner(BarBg, 100)
			
			local Fill = Instance.new("Frame"); Fill.Size=UDim2.new((Padrao - Min)/(Max - Min), 0, 1, 0); Fill.BackgroundColor3=Theme.Accent; Fill.Parent=BarBg
			AddCorner(Fill, 100)
			
			local Circle = Instance.new("Frame"); Circle.Size=UDim2.new(0,14,0,14); Circle.Position=UDim2.new(1,-7,0.5,-7); Circle.BackgroundColor3=Theme.Text; Circle.Parent=Fill
		...
		function Tab:CriarLabel(Texto, Cor)
			local Label = Instance.new("TextLabel"); Label.Text = Texto; Label.Size = UDim2.new(1, 0, 0, 30); Label.BackgroundTransparency = 1
			Label.TextColor3 = Cor or Theme.Text; Label.Font = Enum.Font.GothamBold; Label.TextSize = 14; Label.Parent = TabFrame
			return function(NovoTexto) Label.Text = NovoTexto end
		end

		-- > PROFILE (Cartão de Perfil)
		function Tab:CriarPerfil()
			local Card = Instance.new("Frame"); Card.Size = UDim2.new(1, -10, 0, 120); Card.BackgroundColor3 = Color3.fromRGB(25,25,30); Card.Parent = TabFrame
			AddCorner(Card, 12); AddStroke(Card, Theme.ItemBg, 1)
			
			local Av = Instance.new("ImageLabel"); Av.Size=UDim2.new(0,70,0,70); Av.Position=UDim2.new(0,15,0,25); Av.BackgroundTransparency=1; Av.Parent=Card
			...
			local function AddInfo(t, v, y, c)
				local l = Instance.new("TextLabel"); l.Text=t..": "..v; l.Size=UDim2.new(0.6,0,0,20); l.Position=UDim2.new(0,100,0,y); l.BackgroundTransparency=1
			...
			AddInfo("USUÁRIO", string.upper(Player.Name), 25, Theme.Text)
			AddInfo("ID", Player.UserId, 45, Theme.TextDim)
			AddInfo("STATUS", "VIP ATIVO", 65, Theme.Success)
		end
